...
const getListLiked = () => async (dispatch: AppDispatch) => {
  ...
  const response = await axios.get(
    `http://10.0.2.2:8080/getliked/${getUser.AccountID}`,
  );
  if (response.status) {
    dispatch({
...
const postLiked =
  (body: any, callback: any) => async (dispatch: AppDispatch) => {
    ...
    const response = await axios.post('http://10.0.2.2:8080/liked', body);
...
const delLikeSong =
  (id: number, callback: any) => async (dispatch: AppDispatch) => {
    ...
    const response = await axios.delete(`http://10.0.2.2:8080/liked/${id}`);
